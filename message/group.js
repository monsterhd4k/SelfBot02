...
//const pushname = client.contacts[mem] != undefined ? client.contacts[mem].notify = undefined ? PhoneNumber('+' + mem.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[mem].notify || client.contacts[mem].vname : PhoneNumber('+' + mem.replace('@s.whatsapp.net', '')).getNumber('international')
...
buff = await getBuffer(`http://hadi-api.herokuapp.com/api/welcome3?profile=${pp_user}&name=${pushname}&bg=${Background}&namegb=${encodeURI(groupName)}&member=${groupMembers.length}`)
sendButImage2(from, intro, copyright, buff, butIn)
} else if (Remove && Type2 && onMe) {       
buff = await getBuffer(`http://hadi-api.herokuapp.com/api/goodbye3?profile=${pp_user}&name=${pushname}&bg=${Background}&namegb= ${encodeURI(groupName)}&member=${groupMembers.length}`)
sendButImage(from, outro, copyright, buff, butOut)
}
*/
